...
- AI 的讲解目标不是追求“最小可用实现”，而是解释符合生产工程要求的模块边界、异常处理、幂等性、可测试性和后续扩展点。
...
- 本项目默认按生产方案先行推进，不以“先跑通最小链路”作为教学或设计默认路径。
- 讨论新模块时，应先明确完整架构、模块边界、数据结构、可替换组件、失败场景、测试策略、监控指标和演进路线。
- 最小可执行实现只能作为验证手段或阶段性切片，不能替代生产方案说明。
