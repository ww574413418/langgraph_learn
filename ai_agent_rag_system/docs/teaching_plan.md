...
- 默认按生产方案推进：先讲清楚目标架构、模块边界、工程取舍、异常场景、测试策略和落地顺序，再进入代码。
- 不默认采用“先跑通最小链路 / 最小 demo / 临时版实现”的教学方式；只有学习者明确要求快速验证时，才使用最小可执行路径。
...
每个模块默认先从生产级设计进入，而不是从最小 demo 进入。
...
- 模块在完整系统中的位置。
- 数据结构、接口边界和状态流转。
- 可替换组件，例如 keyword、BM25、vector、rerank、RRF。
- 失败场景、观测指标、测试策略和后续扩展点。
- 最后再拆成可实现步骤。

当某个模块存在临时方案和生产方案时，必须先说明生产方案，再说明是否有必要为了验证而做临时方案。不得默认把“先跑通最小链路”作为主教学路径。
